...
[Claude.ai](https://Claude.ai) and I went back and forth to create a design spec for the system. We ultimately decided to separate the process into ingestion, storage, processing, refinement, and output.

We created a list sources that I would normally search and scan manually. Sites like [LinkedIn](https://linkedin.com/), [BuiltIn](http://builtin.com/), etc. Also, I wanted to identify specific companies "S-tier" that I wanted to pursue working for. Claude (desktop app, Mac) with Chrome integration was used to determine the page structure of each target, and figure out the query structure (e.g. can we take the URL with GET variables after a search, or do we need to submit a form).

### Step 1: The Scraper

The scraper itself is a structure of [python](https://www.python.org/) scripts that take advantage of [Playwright](https://playwright.dev/), which uses a headless browser to navigate the web. While most often used in QA testing, it provides excellent tools to drill down to specific UI components on a web page. Some manual tweaking was necessary to identify in-page structures so that we could retrieve the relevant data from each site.

The scraper performs a few searches on each site for specific sets of keywords, and also searches on the S-tier companies career pages that I specified.

### Step 2: The Database

The listings found are stored in [SQLite](https://sqlite.org/), with typical role data (company, title, salary, location, etc.). Besides these basics, the schema is extended for fields used in subsequent steps.

### Step 3: The Scorer

This is where an 'a-ha' moment really came into play. In another project with Claude, I'm documenting all the computers in my house for reference/maintenance purposes. While we were determining the best local model I could run in [Ollama](https://ollama.com/) on my M4 Mac Mini, I realized I have a powerful Gaming PC sitting here unused all day!

So the PC got Ollama also, and with 64gb of DDR5 RAM (purchased long before the shortage!) and an AMD rx6950xt GPU with 16gb VRAM, it's running [Qwen3.6:35b-a3b](https://qwen.ai/blog?id=qwen3.6-35b-a3b) which is exposed to the Mac via the local (gigabit ethernet) network.
...
For the job postings determined by the scorer to be excellent (85 or higher), they're flagged for an extra 'polish' step to be handled by a more powerful model. In this case, I'm using Claude via [Claude Platform](https://platform.claude.com/) API.
...
I'm keeping a record of all my applications in markdown files in [Obsidian](https://obsidian.md/). The last step for the python scripts is to update a 'Candidates' file for me to manually review.

![A screenshot of an example job captured in the CRM](/images/example-job.png)

## Step Final: The Human

Now, it's up to me to read the summary and click through to each posting and decide whether I'm interested in applying. If a position allows me to include a cover letter, I have a list of items that I should talk about that match my qualifications to the needs expressed in the description.

After I've reviewed and either acted upon or skipped a job, I move that entry to an Archive file for reference later if I do get a call back and want to check the notes for that role.
